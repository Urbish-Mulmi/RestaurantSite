...
// admin-only: list every user, newest first, password excluded
...
export const updateUserRole = async (req, res) => {
  try {
    ...
    // guard: admin can't change their own role, avoids accidental self-lockout
...
export const deleteUser = async (req, res) => {
  try {
    ...
    // guard: admin can't delete their own account
    if (req.user._id.toString() === id) {
      return res.status(400).json({
